...
export function ItemEditorDialog({ item, open, onClose, canViewFinancials = false }: ItemEditorDialogProps) {
  ...
  const computedCostMxnEq = useMemo(() => {
    ...
    if (amount !== null && rate !== null && Number.isFinite(amount) && Number.isFinite(rate)) {
      return amount * rate;
    }
    if (item && item.costMxnEq != null) {
      return item.costMxnEq;
    }
    return null;
  }, [form.costAmount, form.exchangeRate, item]);
  ...
  // Sync exchange rate from store when it loads
  useEffect(() => {
    if (currentRate && activeTab === "compra" && form.exchangeRate === "" && item?.exchangeRate == null) {
      setForm((current) => ({ ...current, exchangeRate: currentRate.rate.toString() }));
    }
  }, [currentRate, activeTab, form.exchangeRate, item?.exchangeRate]);
        ...
        <form onSubmit={submit}>
          <Tabs value={activeTab} onValueChange={setActiveTab} className="mt-2">
            <TabsList className="w-full">
              ...
              <TabsTrigger value="compra" className="flex-1">Compra</TabsTrigger>
            ...
            <TabsContent value="compra" className="space-y-5">
              <section className="space-y-3">
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  <Field label="Costo (USD)" htmlFor="editor-cost-amount" error={errors.costAmount}>
